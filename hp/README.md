...
├── build/                       記事ビルド（記事ページ・一覧・sitemap.xml・robots.txt を生成）※公開サーバーでは非公開
├── column/ knowledge/ notice/   記事ページとカテゴリ一覧（../content/articles/ から自動生成。直接編集しない）
├── cases/                       成功事例。業種ページと解決事例ページは自動生成（/cases/ の一覧と飲食店ページは手作業のHTML）
...
- **ページ文字編集**（記事ページ・カテゴリ一覧以外の全ページ）: 「編集開始」→ 文字を直接書き換え →「保存してHPに反映」。
- **記事エディタ** `http://localhost:8080/admin/editor/`（コラム `column/`・基礎知識 `knowledge/`・お知らせ `notice/`・成功事例 `cases/`）。詳しくは下の「記事（Markdown → 静的HTML）」。
  - 左の「記事一覧」で検索・カテゴリ絞り込み。「＋ 新しい記事を作る」で作成（成功事例は業種を選ぶとURL名を自動で提案）。
  - 本文はブロック単位（段落・見出し・箇条書き・画像・表・引用・注意書きなど）。文字を選ぶと書式ボタンが出ます（太字 Ctrl+B・リンク Ctrl+K。リンクはサイト内のページを検索して選べます）。
  - 画像はボタンで選ぶか本文にドラッグ＆ドロップ。大きい写真は自動で縮小され、`assets/img/articles/<カテゴリ>-<URL名>/` に保存されます。
  - 「プレビュー」で保存前の見た目を別タブで確認。編集中の内容はブラウザに一時保存され、閉じてしまっても次に開いたとき戻せます。
  - 「検索結果・SNSでの見え方」でGoogleの表示イメージとアイキャッチ画像（SNS共有時の画像）を設定できます。
  - 新規作成（最初は下書き）、公開日・タグ・下書き（非公開）の設定
  - 記事タイトル、見出し下の説明文、英字ラベル、SEOタイトル、メタディスクリプション、一覧ページの紹介文（お知らせは分類）
...
- **変更履歴**: 保存のたびに変更前の状態（記事は元データの Markdown、それ以外のページは HTML）が `_backups/` に残り（1ファイル100件まで）、ツールバーの「変更履歴」から復元できます。`_backups/` はgit管理外です。
- 編集APIはこのPC（localhost）からのアクセスのみ受け付けます。LAN内の別端末から編集する場合は `ALLOW_REMOTE_EDIT=1` を付けて起動してください。
- 注意: `_generate.py` でページを再生成すると、ページ文字編集で直した内容は上書きされます（記事ページ・カテゴリ一覧・成功事例の業種／解決事例ページは `_generate.py` の対象外です。また、原稿を反映した「当センターについて・代表者挨拶・解決プロセス・料金・サービス内容・よくあるご質問」の6ページは、ファイルがあれば上書きしません）。

## 記事（Markdown → 静的HTML）

コラム・基礎知識・お知らせの記事と、成功事例（業種ページ・解決事例ページ）は、リポジトリ直下の `content/articles/<カテゴリ>/<URL名>.md` が元データです。ここから `build/articles.js` が静的HTMLを生成します。記事エディタで保存すると、その場でビルドまで行われます。

```powershell
# 手作業で .md を直したときは、これで生成し直せます
cd hp
...
本文。**太字**や[リンク](../../contact/)も書けます。
```

| 項目 | 内容 |
|---|---|
| `title` / `description` / `published` / `updated` | 必須（`updated` は記事エディタで保存すると自動で今日の日付になります） |
| `tags` | 関連記事の選定に使います |
| `lead` | 見出し下の説明文（省略時は `description`） |
| `summary` | 一覧ページの紹介文（省略時は `description`） |
| `seoTitle` | `<title>`（省略時は「タイトル｜カテゴリ｜一般社団法人口コミ対策センター」） |
| `image` | OGP・一覧のサムネイル（省略時は `/assets/img/ogp-default.jpg`） |
| `author` | 著者名（省略時は当センター） |
| `noticeType` | お知らせのみ：お知らせ／注意喚起／コラム |
| `draft: true` | 下書き（公開しない。以前公開していたページは取り下げ） |
| `parent` | 成功事例のみ：親にする業種ページのURL名（例 `realestate`）。親ページに解決事例の一覧が並び、パンくずにも親が入ります |
| `order` | 親ページの一覧での並び順（小さい順。「事例1」「事例2」…の番号になります） |
| `listHeading` | 親ページに並ぶ一覧の見出し（例「不動産業界の解決事例」） |
| `navTitle` | パンくず・「戻る」リンクに使う短い名前（タイトルが長いページ向け。省略時はタイトル） |

### 生成されるもの

- **記事ページ** `/<カテゴリ>/<URL名>/`：`<title>`・description・canonical・OGP（og:image を含む）・構造化データ（BlogPosting とパンくず）・公開日と更新日・タグ・関連記事（共通タグ→同じカテゴリ→新しい順で3件）・カテゴリ一覧への内部リンク。h1 は記事タイトルの1つだけです。
- **成功事例** `/cases/<業種>/`（業種ページ＋解決事例の一覧）と `/cases/<業種>-case-<番号>/`（解決事例）。`/cases/` の一覧ページは自動生成しません。
- **カテゴリ一覧** `/column/`・`/knowledge/`・`/notice/`：新しい順の一覧と、CollectionPage・パンくずの構造化データ。
- **`sitemap.xml`**：公開対象の全ページ（記事・一覧には更新日つき）。転送ページ・noindex・完了ページは除外します。
- **`robots.txt`**：全体を許可し、`/admin/`・`/backend/`・`/build/` を除外して、サイトマップの場所を記載します。

ヘッダー・フッターなどの共通部分は、既存の記事ページ／一覧ページのHTMLをそのまま引き継ぎます。作り直すのは `<head>` のSEOタグと `<main>` の中身だけです。
