...
```powershell
cd "C:\Users\meari\OneDrive\デスクトップ\mearin0424-tech\kuchikomi\hp-renewal"
node server.js
```
...
3. **サイトのコピー**（`hp-renewal` フォルダを OneDrive の外に複製したもの）で、次を実行します。
   ```powershell
...
- フォームのデザイン・流れ: `form/`・`diagnosis-form/`・`lp/biz-hidden-gad/` は公開中LPと同じ2段階フォーム（HPのURL →「診断開始」→ 連絡先）。見た目は `assets/css/lp-form.css`、メールか電話の「いずれか必須」チェックや2段階の動きは `assets/js/track.js` にあります。
...
3. サイト一式を `public_html` にアップロードします。`_config/`・`_backups/` はアップロードしないでください（`admin/` などは `.htaccess` で公開されないようにしてあります）。
